...
export async function createAndLoginUser(app: INestApplication, username: string = `test_${Date.now()}`) {
  ...
  try {
    ...
    if (signupRes.status !== 201) {
      console.log(`User registration failed with status: ${signupRes.status}`);
    }
    ...
    if (loginRes.status !== 201) {
      console.log(`User login failed with status: ${loginRes.status}`);
      return { user: null, authCookie: null };
    }
